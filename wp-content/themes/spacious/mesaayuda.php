...
if($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	if(in_array($_POST['prioridad'], $prioridades)){
		...
		if (isset($_FILES['adjunto1']) && $_FILES['adjunto1']['name']!="") 
		{
			...
			{
				$valido= 0;
				$error.="El archivo adjunto nro. 1 debe ser inferior a 1 MB";
			}
				...
				)
			{
				...
				$valido= 0;
				$error.= "Tipos de archivos aceptados: *.bmp, *.gif, *.jpg, *.png, *.zip, *.rar, *.csv, *.doc, *.docx, *.xls, *.xlsx, *.txt, *.pdf ";
			}
		} else 
			{
				$valido= 0;
		...
		if (isset($_FILES['adjunto2']) && $_FILES['adjunto2']['name']!="") 
		{
                        ...
                        {
                                $valido= 0;
                                $error.="El archivo adjunto nro. 2 debe ser inferior a 1 MB";
                        }
			...
			if (($_FILES["adjunto2"]["type"] == "application/pdf")
                                ...
                                || ($_FILES["adjunto2"]["type"] == "image/x-windows-bmp") || ($_FILES["adjunto2"]["type"] == "application/zip")
                                || ($_FILES["adjunto2"]["type"] == "application/x-compressed") || ($_FILES["adjunto2"]["type"] == "application/x-zip-compressed")
                                || ($_FILES["adjunto2"]["type"] == "multipart/x-zip") || ($_FILES["adjunto2"]["type"] == "application/x-rar-compressed")
                                || ($_FILES["adjunto2"]["type"] == "application/octet-stream") || ($_FILES["adjunto2"]["type"] == "appication/csv")
                                || ($_FILES["adjunto2"]["type"] == "text/csv") || ($_FILES["adjunto2"]["type"] == "application/x-excel")
                                || ($_FILES["adjunto2"]["type"] == "application/x-msexcel")) 
			
			{
				...
				$valido= 0;
				$error.= "Tipos de archivos aceptados: *.bmp, *.gif, *.jpg, *.png, *.zip, *.rar, *.csv, *.doc, *.docx, *.xls, *.xlsx, *.txt, *.pdf <br>";
			}
		}
